...
parser.add_argument('--model_path', default='save_model/sysu_base_p4_n6_lr_0.2_seed_0_best', type=str,
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat_pool, feat_fc = net(input, test_mode[0])
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat_fc = net(input, test_mode[1])
...
if dataset == 'sysu':
    ...
    result = {'gallery_f': gall_feat_fc, 'gallery_label': gall_label, 'gallery_cam': gall_cam,
                  'query_f': query_feat_fc, 'query_label': query_label, 'query_cam': query_cam}
